...
import { LucideCircleDollarSign, TextSelect } from 'lucide-react'
...
export default function ProjectCard({ project, onAssignmentCreated }: ProjectProps) {
    const total = project.ProjectAssignment?.length ?? 0;
    const completed = project.ProjectAssignment?.filter(a => a.status === "COMPLETO").length ?? 0;
    ...
    `}
            ...
            <CardHeader>
                <CardTitle className="flex justify-between items-center">
                    <div className="whitespace-normal font-bold flex flex-col gap-1">
                        <h1 className="flex items-center gap-1"> <TextSelect className="h-4"></TextSelect> {project.name}</h1>
                        <CardDescription><h3 className="font-medium text-sm">  {project.field}</h3></CardDescription>
                    </div>
                ...
                <CardDescription className="mt-3">
                    ...
                    <div className="flex flex-row mt-5 justify-between items-center">
                        ...
                        <div>
                            ...
                            <h1>
                                {project.deadline ? formatDateUTC(String(project.deadline)) : "—"}
                            </h1>
                        </div>
