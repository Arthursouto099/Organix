...
export async function createRelationCollaborator(projectId:string, userId: string, task: string, description: string, deadline: string) {
    const request = await fetch(`${import.meta.env.VITE_API_URL}/assignment/create/${projectId}`, {
        method: "POST",
        headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({userId, task, description,  deadline: new Date(deadline).toISOString()})
    })
